...
export function InteractionForm({ interaction, companies, contacts, teamMembers, defaultCompanyId }: InteractionFormProps) {
  ...
  const form = useForm<InteractionFormValues>({
    ...
    defaultValues: {
      ...
      meeting_datetime: interaction?.meeting_datetime ? interaction.meeting_datetime.slice(0, 16) : "",
  ...
  function onSubmit(values: InteractionFormValues, mode: "save" | "addAnother" = "save") {
    ...
    startTransition(async () => {
      ...
      if (mode === "addAnother" && !interaction) {
        ...
        form.reset({ company_id: companyId, contact_person_id: "", assigned_user_id: "", interaction_type: "Phone Call", meeting_datetime: "", discussion_details: "", need_help: false, status: "active" });
  ...
  return (
    <form className="space-y-5" onSubmit={form.handleSubmit((values) => onSubmit(values, "save"))}>
      <p className="rounded-md border bg-white p-3 text-sm text-muted-foreground">Only company and discussion details are required. You can add more information later.</p>
      <FormSection title="Basic Information" description="Required company context and discussion notes.">
        ...
        <Field label="Discussion Details" required error={form.formState.errors.discussion_details?.message}>
          <textarea {...form.register("discussion_details")} className="min-h-28 w-full rounded-md border bg-background px-3 py-2 text-sm" />
        </Field>
      </FormSection>
      ...
      <CollapsibleSection title="Internal Notes" description="Internal team context and help flags.">
        ...
        <div className="md:col-span-2 xl:col-span-4"><Label>Internal Note</Label><textarea {...form.register("internal_note")} className="mt-2 min-h-28 w-full rounded-md border bg-background px-3 py-2 text-sm" /></div>
      </CollapsibleSection>
...
function CollapsibleSection({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <details className="rounded-lg border bg-card shadow-soft"><summary className="cursor-pointer list-none p-5"><h3 className="font-semibold">{title}</h3><p className="mt-1 text-sm text-muted-foreground">{description}</p></summary><div className="grid gap-4 p-5 pt-0 md:grid-cols-2 xl:grid-cols-4">{children}</div></details>;
}
...
function SelectField({ label, required, error, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; required?: boolean; error?: string }) {
  return <div className="space-y-2"><Label>{label}{required ? <span className="text-destructive"> *</span> : null}</Label><select {...props} className="h-10 w-full rounded-md border bg-background px-3 text-sm">{children}</select>{error ? <p className="text-xs text-destructive">{error}</p> : null}</div>;
}
